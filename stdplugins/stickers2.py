...
@borg.on(admin_cmd(pattern="sajat ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    me = borg.me
    userid = borg.uid
    packname = f"Nasi Kerabu Pack"
    packshortname = f"Id_267053545_p2"  # format: Uni_Borg_userid
    ...
    if is_a_s:
        ...
        packname = f"Yong Tau Foo"
        packshortname = f"Id_267053545_as"  # format: Uni_Borg_userid
